...
function Creators({ data, setData, navigate, myView }) {
  ...
  const filtered = data.creators.filter(c => {
    ...
    const matchCampaign = !filterCampaign || c.campaign_id === filterCampaign;
    const matchOwner = !myView || c.owner_id === data.current_user_id;
    return matchSearch && matchStatus && matchProduct && matchPlatform && matchSource && matchCampaign && matchOwner;
  });
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select value={filterCampaign} onChange={e => setFilterCampaign(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:border-lime-400 transition-all">
            <option value="">All campaigns</option>
            {data.campaigns.map(c => <option key={c.id} value={c.id}>{c.campaign_name}</option>)}
          </select>
          <button
          ...
          {(search || filterStatus || filterProduct || filterPlatform || filterSource || filterCampaign) && (
            <Btn size="sm" variant="ghost" onClick={() => { setSearch(''); setFilterStatus(''); setFilterProduct(''); setFilterPlatform(''); setFilterSource(''); setFilterCampaign(''); }}>Clear</Btn>
          )}
